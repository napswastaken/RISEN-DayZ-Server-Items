////////////////////////////////////////////////////////////////////
//DeRap: Nulled_Firearms\vssk\config.bin
...
{
	class Rifle_Base;
	class VSSK_Base: Rifle_Base
	{
		scope = 0;
		weight = 1840;
		noiseShootModifier = -5.8;
		absorbency = 0.0;
		...
		chamberedRound = "";
		chamberSize = 1;
		chamberableFrom[] = {"NULLED_Ammo_127x55_SC_130VPS"};
		magazines[] = {"Mag_VSSK_5Rnd"};
		...
		{
			soundSetShot[] = {"VSSK_Shot_SoundSet","AK_silencerTail_SoundSet","AK_silencerInteriorTail_SoundSet"};
	...
	{
		scope = 2;
		displayName = "VKs/VSSK Vychlop";
		descriptionShort = "This Russian sniper rifle resembles quite a bit to the AR15 Potato launchers, but that’s about where the similarities end as well. VSSK stands for Vintovka Snayperskaya Spetsialnaya Krupnokalibernaya, so thank’s to the Russians for abbreviating that. It’s a 12.7×55 mm (.50) large caliber special sniper rifle made by the BFP Instrument Design Bureau, with focus on military and police special operations.The design is a bullpup, operated by a straight pull, bolt action with a rotary locking bolt head. Note that the case length is only 55 mm, so considerably shorter than a .50 BMG (12,7×99 mm) or a Russian 12.7 mm x 108 mm. The projectiles are heavy, from 910 grains (59 gram) to 1170 grains (76 gram). That’s a whole lot of sub-sonic something coming out of the barrel and the suppressor, but the velocities are somewhat low at around 950 feet per second (needs to be sub-sonic).";
		model = "Firearms\VSSK\VSSK.p3d";
